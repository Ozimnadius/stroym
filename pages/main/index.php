<main class="main">

    <!-- banner -->
    <section class="banner">

        <div class="swiper-container banner__container">
            <div class="swiper-wrapper">
                <div class="swiper-slide banner__slide" style="background-image: url(/images/bg/bg-banner.jpg);">
                    <div class="container">
                        <div class="row">
                            <div class="banner__slide-left">
                                <div class="banner__slide-content content">
                                    <svg class="banner__slide-title" width="265" height="66"
                                         xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 265 50">
                                        <text x="0" y="40">Скидка</text>
                                    </svg>
                                    <div class="banner__slide-percent">
                                        -25%
                                    </div>
                                    <div class="banner__slide-text">
                                        на лампы filament
                                    </div>
                                </div>
                                <a role="link" href="" title="Подробнее об акции" class="button banner__slide-button">
                                    <span class="button__text">Подробнее об акции</span>
                                </a>

                            </div>
                        </div>
                    </div>
                </div>
                <div class="swiper-slide banner__slide" style="background-image: url(/images/bg/bg-banner.jpg);">
                    <div class="container">
                        <div class="row">
                            <div class="banner__slide-left">
                                <div class="banner__slide-content content">
                                    <svg class="banner__slide-title" width="265" height="66"
                                         xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 265 50">
                                        <text x="0" y="40">Скидка</text>
                                    </svg>
                                    <div class="banner__slide-percent">
                                        -25%
                                    </div>
                                    <div class="banner__slide-text">
                                        на лампы filament
                                    </div>
                                </div>
                                <a role="link" href="" title="Подробнее об акции" class="button banner__slide-button">
                                    <span class="button__text">Подробнее об акции</span>
                                </a>

                            </div>
                        </div>
                    </div>
                </div>
                <div class="swiper-slide banner__slide" style="background-image: url(/images/bg/bg-banner.jpg);">
                    <div class="container">
                        <div class="row">
                            <div class="banner__slide-left">
                                <div class="banner__slide-content content">
                                    <svg class="banner__slide-title" width="265" height="66"
                                         xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 265 50">
                                        <text x="0" y="40">Скидка</text>
                                    </svg>
                                    <div class="banner__slide-percent">
                                        -25%
                                    </div>
                                    <div class="banner__slide-text">
                                        на лампы filament
                                    </div>
                                </div>
                                <a role="link" href="" title="Подробнее об акции" class="button banner__slide-button">
                                    <span class="button__text">Подробнее об акции</span>
                                </a>

                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="banner__nav">
            <div class="container">
                <div class="banner__nav-container">

                    <!-- pagination -->
                    <div class="swiper-pagination banner__pagination"></div>
                    <!-- END pagination -->

                    <!-- arrows -->
                    <div class="banner__arrows">
                        <div class="banner__prev banner__arrow">
                            <svg class="banner__svg">
                                <use xlink:href="images/icons/sprite.svg#left"></use>
                            </svg>
                        </div>
                        <div class="banner__next banner__arrow">
                            <svg class="banner__svg">
                                <use xlink:href="images/icons/sprite.svg#right"></use>
                            </svg>
                        </div>
                    </div>
                    <!-- END arrows -->
                </div>
            </div>
        </div>

    </section>
    <!-- END banner -->

    <!-- tabs -->
    <section class="main-tabs">
        <div class="container">

            <!-- accordeon -->
            <div class="acco">
                <div class="acco__item">
                    <div class="acco__button">
                        <div class="acco__button-wrapper">
                            <svg class="acco__button-svg">
                                <use xlink:href="images/icons/sprite.svg#main"></use>
                            </svg>
                            <span class="acco__button-text">
                                Главная
                            </span>
                            <div class="acco__button-symbol">
                                <div class="acco__button-line"></div>
                                <div class="acco__button-line"></div>
                            </div>
                        </div>
                    </div>
                    <div class="acco__content">
                        <div class="main-cat cats">
                            <div class="row">
                                <div class="cat">
                                    <!-- link -->
                                    <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                        Светильники светодиодные
                                    </a>
                                    <!-- END link -->

                                    <!-- img -->
                                    <div class="cat__img">
                                        <img src="images/content/cats/cat1.png" alt="Светильники светодиодные"
                                             title="Светильники светодиодные" class="cat__pic">
                                    </div>
                                    <!-- END img -->

                                    <div class="cat__bg"
                                         style="background-image: url(images/content/cats/catbg1.jpg);"></div>

                                    <!-- subcats -->
                                    <div class="subcats">
                                        <div class="subcats__blur"
                                             style="background-image: url(images/content/cats/catbg1.jpg);">
                                            <div class="cat__img">
                                                <img src="images/content/cats/cat1.png" alt="Светильники светодиодные"
                                                     title="Светильники светодиодные" class="cat__pic">
                                            </div>
                                        </div>
                                        <div class="subcats__content">
                                            <a role="link" href="category.php" title="Светильники светодиодные"
                                               class="subcats__title">
                                                Светильники светодиодные
                                            </a>
                                            <div class="subcats__list">
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы"
                                                   class="subcat subcat_link link">
                                                <span class="link__text">
                                                    Ещё 13
                                                </span>
                                                    <svg class="link__svg">
                                                        <use xlink:href="images/icons/sprite.svg#right"></use>
                                                    </svg>
                                                </a>

                                            </div>
                                        </div>
                                    </div>
                                    <!-- END subcats -->

                                </div>
                                <div class="cat">
                                    <!-- link -->
                                    <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                        Светильники светодиодные
                                    </a>
                                    <!-- END link -->

                                    <!-- img -->
                                    <div class="cat__img">
                                        <img src="images/content/cats/cat2.png" alt="Светильники светодиодные"
                                             title="Светильники светодиодные" class="cat__pic">
                                    </div>
                                    <!-- END img -->

                                    <div class="cat__bg"
                                         style="background-image: url(images/content/cats/catbg2.jpg);"></div>

                                    <!-- subcats -->
                                    <div class="subcats">
                                        <div class="subcats__blur"
                                             style="background-image: url(images/content/cats/catbg2.jpg);">
                                            <div class="cat__img">
                                                <img src="images/content/cats/cat2.png" alt="Светильники светодиодные"
                                                     title="Светильники светодиодные" class="cat__pic">
                                            </div>
                                        </div>
                                        <div class="subcats__content">
                                            <a role="link" href="category.php" title="Светильники светодиодные"
                                               class="subcats__title">
                                                Светильники светодиодные
                                            </a>
                                            <div class="subcats__list">
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы"
                                                   class="subcat subcat_link link">
                                                <span class="link__text">
                                                    Ещё 13
                                                </span>
                                                    <svg class="link__svg">
                                                        <use xlink:href="images/icons/sprite.svg#right"></use>
                                                    </svg>
                                                </a>

                                            </div>
                                        </div>
                                    </div>
                                    <!-- END subcats -->

                                </div>
                                <div class="cat cat_3">
                                    <!-- link -->
                                    <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                        Лампы
                                    </a>
                                    <!-- END link -->

                                    <!-- img -->
                                    <div class="cat__img">
                                        <img src="images/content/cats/cat3.png" alt="Светильники светодиодные"
                                             title="Светильники светодиодные" class="cat__pic">
                                    </div>
                                    <!-- END img -->

                                    <div class="cat__bg"
                                         style="background-image: url(images/content/cats/catbg3.jpg);"></div>

                                    <!-- subcats -->
                                    <div class="subcats">
                                        <div class="subcats__blur"
                                             style="background-image: url(images/content/cats/catbg3.jpg);">
                                            <div class="cat__img">
                                                <img src="images/content/cats/cat3.png" alt="Светильники светодиодные"
                                                     title="Светильники светодиодные" class="cat__pic">
                                            </div>
                                        </div>
                                        <div class="subcats__content">
                                            <a role="link" href="category.php" title="Светильники светодиодные"
                                               class="subcats__title">
                                                Лампы
                                            </a>
                                            <div class="subcats__list">
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы"
                                                   class="subcat subcat_link link">
                                                <span class="link__text">
                                                    Ещё 13
                                                </span>
                                                    <svg class="link__svg">
                                                        <use xlink:href="images/icons/sprite.svg#right"></use>
                                                    </svg>
                                                </a>

                                            </div>
                                        </div>
                                    </div>
                                    <!-- END subcats -->

                                </div>
                                <div class="cat cat_3">
                                    <!-- link -->
                                    <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                        Лампы
                                    </a>
                                    <!-- END link -->

                                    <!-- img -->
                                    <div class="cat__img">
                                        <img src="images/content/cats/cat4.png" alt="Светильники светодиодные"
                                             title="Светильники светодиодные" class="cat__pic">
                                    </div>
                                    <!-- END img -->

                                    <div class="cat__bg"
                                         style="background-image: url(images/content/cats/catbg4.jpg);"></div>

                                    <!-- subcats -->
                                    <div class="subcats">
                                        <div class="subcats__blur"
                                             style="background-image: url(images/content/cats/catbg4.jpg);">
                                            <div class="cat__img">
                                                <img src="images/content/cats/cat4.png" alt="Светильники светодиодные"
                                                     title="Светильники светодиодные" class="cat__pic">
                                            </div>
                                        </div>
                                        <div class="subcats__content">
                                            <a role="link" href="category.php" title="Светильники светодиодные"
                                               class="subcats__title">
                                                Лампы
                                            </a>
                                            <div class="subcats__list">
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы"
                                                   class="subcat subcat_link link">
                                                <span class="link__text">
                                                    Ещё 13
                                                </span>
                                                    <svg class="link__svg">
                                                        <use xlink:href="images/icons/sprite.svg#right"></use>
                                                    </svg>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                    <!-- END subcats -->

                                </div>
                                <div class="cat cat_3">
                                    <!-- link -->
                                    <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                        Лампы
                                    </a>
                                    <!-- END link -->

                                    <!-- img -->
                                    <div class="cat__img">
                                        <img src="images/content/cats/cat5.png" alt="Светильники светодиодные"
                                             title="Светильники светодиодные" class="cat__pic">
                                    </div>
                                    <!-- END img -->

                                    <div class="cat__bg"
                                         style="background-image: url(images/content/cats/catbg5.jpg);"></div>

                                    <!-- subcats -->
                                    <div class="subcats">
                                        <div class="subcats__blur"
                                             style="background-image: url(images/content/cats/catbg5.jpg);">
                                            <div class="cat__img">
                                                <img src="images/content/cats/cat5.png" alt="Светильники светодиодные"
                                                     title="Светильники светодиодные" class="cat__pic">
                                            </div>
                                        </div>
                                        <div class="subcats__content">
                                            <a role="link" href="category.php" title="Светильники светодиодные"
                                               class="subcats__title">
                                                Лампы
                                            </a>
                                            <div class="subcats__list">
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы"
                                                   class="subcat subcat_link link">
                                                <span class="link__text">
                                                    Ещё 13
                                                </span>
                                                    <svg class="link__svg">
                                                        <use xlink:href="images/icons/sprite.svg#right"></use>
                                                    </svg>
                                                </a>

                                            </div>
                                        </div>
                                    </div>
                                    <!-- END subcats -->

                                </div>
                                <div class="cat lg-12">
                                    <!-- link -->
                                    <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                        Светильники светодиодные
                                    </a>
                                    <!-- END link -->

                                    <div class="cat__bg"
                                         style="background-image: url(images/content/cats/catbg6.jpg);"></div>

                                    <!-- subcats -->
                                    <div class="subcats">
                                        <div class="subcats__blur"
                                             style="background-image: url(images/content/cats/catbg6.jpg);"></div>
                                        <div class="subcats__content">
                                            <a role="link" href="category.php" title="Светильники светодиодные"
                                               class="subcats__title">
                                                Светильники светодиодные
                                            </a>
                                            <div class="subcats__list">
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы"
                                                   class="subcat subcat_link link">
                                                    <span class="link__text">
                                                         Ещё 13
                                                    </span>
                                                    <svg class="link__svg">
                                                        <use xlink:href="images/icons/sprite.svg#right"></use>
                                                    </svg>
                                                </a>

                                            </div>
                                        </div>
                                    </div>
                                    <!-- END subcats -->

                                </div>
                                <div class="cat">
                                    <!-- link -->
                                    <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                        Светильники светодиодные
                                    </a>
                                    <!-- END link -->


                                    <div class="cat__bg"
                                         style="background-image: url(images/content/cats/catbg7.jpg);"></div>

                                    <!-- subcats -->
                                    <div class="subcats">
                                        <div class="subcats__blur"
                                             style="background-image: url(images/content/cats/catbg7.jpg);"></div>
                                        <div class="subcats__content">
                                            <a role="link" href="category.php" title="Светильники светодиодные"
                                               class="subcats__title">
                                                Светильники светодиодные
                                            </a>
                                            <div class="subcats__list">
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                    Энергосберигающие лампы
                                                </a>
                                                <a role="link" href="category.php" title="Энергосберигающие лампы"
                                                   class="subcat subcat_link link">
                                                    <span class="link__text">
                                                        Ещё 13
                                                    </span>
                                                    <svg class="link__svg">
                                                        <use xlink:href="images/icons/sprite.svg#right"></use>
                                                    </svg>
                                                </a>

                                            </div>
                                        </div>
                                    </div>
                                    <!-- END subcats -->

                                </div>
                            </div>

                            <a role="link" href="catalog.php" class="cats__more link">
                                <span class="link__text">
                                    Смотреть весь каталог
                                </span>
                                <svg class="link__svg">
                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                </svg>
                            </a>
                        </div>
                    </div>
                </div>
                <div class="acco__item">
                    <div class="acco__button">
                        <div class="acco__button-wrapper">
                            <svg class="acco__button-svg">
                                <use xlink:href="images/icons/sprite.svg#catalog"></use>
                            </svg>
                            <div class="acco__button-text">
                    Каталог
                </div>
                            <div class="acco__button-symbol">
                                <div class="acco__button-line"></div>
                                <div class="acco__button-line"></div>
                            </div>
                        </div>
                    </div>
                    <div class="acco__content">
                        <div class="main-catalog">
                            <div class="folders">
                                <div class="row">
                                    <div class="folder">
                                        <a href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                            <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                        </a>

                                        <div class="subfolders">
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder link"
                                               title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>
                                    <div class="folder">
                                        <a role="link" href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                            <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                        </a>

                                        <div class="subfolders">
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder link"
                                               title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>
                                    <div class="folder">
                                        <a role="link" href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                            <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                        </a>

                                        <div class="subfolders">
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder link"
                                               title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>
                                    <div class="folder">
                                        <a role="link" href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                            <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                        </a>

                                        <div class="subfolders">
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder link"
                                               title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>
                                    <div class="folder">
                                        <a role="link" href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                            <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                        </a>

                                        <div class="subfolders">
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder link"
                                               title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>
                                    <div class="folder">
                                        <a role="link" href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                            <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                        </a>

                                        <div class="subfolders">
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" class="subfolder link"
                                               title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>

                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="acco__item">
                    <div class="acco__button">
                        <div class="acco__button-wrapper">
                            <svg class="acco__button-svg">
                                <use xlink:href="images/icons/sprite.svg#actions"></use>
                            </svg>
                            <div class="acco__button-text">
                                Акции
                            </div>
                            <div class="acco__button-symbol">
                                <div class="acco__button-line"></div>
                                <div class="acco__button-line"></div>
                            </div>
                        </div>
                    </div>
                    <div class="acco__content">
                        <? include 'pages/common/actions.php' ?>
                    </div>
                </div>
            </div>
            <!-- END accordeon -->

            <!-- switches -->
            <div class="switches">
                <div class="row">
                    <div class="switches__list">
                        <a href="#tab1" role="button" class="switch active">
                            <svg class="switch__svg">
                                <use xlink:href="images/icons/sprite.svg#main"></use>
                            </svg>
                            <span class="switch__text">
                                Главная
                            </span>
                        </a>
                        <a href="#tab2" role="button" class="switch">
                            <svg class="switch__svg">
                                <use xlink:href="images/icons/sprite.svg#catalog"></use>
                            </svg>
                            <span class="switch__text">
                    Каталог
                </span>
                        </a>
                        <a href="#tab3" role="button" class="switch">
                            <svg class="switch__svg">
                                <use xlink:href="images/icons/sprite.svg#actions"></use>
                            </svg>
                            <span class="switch__text">
                    Акции
                </span>
                        </a>
                    </div>
                </div>
            </div>
            <!-- END switches -->

            <!-- tabs -->
            <div class="tabs">
                <div class="tab active" id="tab1">
                    <svg hidden xmlns="http://www.w3.org/2000/svg">
                        <filter id="blur">
                            <feGaussianBlur in="SourgeGraphic" stdDeviation="2"></feGaussianBlur>
                        </filter>
                    </svg>
                    <div class="main-cat cats">
                        <div class="row">
                            <div class="cat">
                                <!-- link -->
                                <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                    Светильники светодиодные
                                </a>
                                <!-- END link -->

                                <!-- img -->
                                <div class="cat__img">
                                    <img src="images/content/cats/cat1.png" alt="Светильники светодиодные"
                                         title="Светильники светодиодные" class="cat__pic">
                                </div>
                                <!-- END img -->

                                <div class="cat__bg"
                                     style="background-image: url(images/content/cats/catbg1.jpg);"></div>

                                <!-- subcats -->
                                <div class="subcats">
                                    <div class="subcats__blur"
                                         style="background-image: url(images/content/cats/catbg1.jpg);">
                                        <div class="cat__img">
                                            <img src="images/content/cats/cat1.png" alt="Светильники светодиодные"
                                                 title="Светильники светодиодные" class="cat__pic">
                                        </div>
                                    </div>
                                    <div class="subcats__content">
                                        <a href="category.php" title="Светильники светодиодные" class="subcats__title">
                                            Светильники светодиодные
                                        </a>
                                        <div class="subcats__list">
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы"
                                               class="subcat subcat_link link">
                                                <span class="link__text">
                                                    Ещё 13
                                                </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>
                                </div>
                                <!-- END subcats -->

                            </div>
                            <div class="cat">
                                <!-- link -->
                                <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                    Светильники светодиодные
                                </a>
                                <!-- END link -->

                                <!-- img -->
                                <div class="cat__img">
                                    <img src="images/content/cats/cat2.png" alt="Светильники светодиодные"
                                         title="Светильники светодиодные" class="cat__pic">
                                </div>
                                <!-- END img -->

                                <div class="cat__bg"
                                     style="background-image: url(images/content/cats/catbg2.jpg);"></div>

                                <!-- subcats -->
                                <div class="subcats">
                                    <div class="subcats__blur"
                                         style="background-image: url(images/content/cats/catbg2.jpg);">
                                        <div class="cat__img">
                                            <img src="images/content/cats/cat2.png" alt="Светильники светодиодные"
                                                 title="Светильники светодиодные" class="cat__pic">
                                        </div>
                                    </div>
                                    <div class="subcats__content">
                                        <a role="link" href="category.php" title="Светильники светодиодные" class="subcats__title">
                                            Светильники светодиодные
                                        </a>
                                        <div class="subcats__list">
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы"
                                               class="subcat subcat_link link">
                                                <span class="link__text">
                                                    Ещё 13
                                                </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>
                                </div>
                                <!-- END subcats -->

                            </div>
                            <div class="cat cat_3">
                                <!-- link -->
                                <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                    Лампы
                                </a>
                                <!-- END link -->

                                <!-- img -->
                                <div class="cat__img">
                                    <img src="images/content/cats/cat3.png" alt="Светильники светодиодные"
                                         title="Светильники светодиодные" class="cat__pic">
                                </div>
                                <!-- END img -->

                                <div class="cat__bg"
                                     style="background-image: url(images/content/cats/catbg3.jpg);"></div>

                                <!-- subcats -->
                                <div class="subcats">
                                    <div class="subcats__blur"
                                         style="background-image: url(images/content/cats/catbg3.jpg);">
                                        <div class="cat__img">
                                            <img src="images/content/cats/cat3.png" alt="Светильники светодиодные"
                                                 title="Светильники светодиодные" class="cat__pic">
                                        </div>
                                    </div>
                                    <div class="subcats__content">
                                        <a role="link" href="category.php" title="Светильники светодиодные" class="subcats__title">
                                            Лампы
                                        </a>
                                        <div class="subcats__list">
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы"
                                               class="subcat subcat_link link">
                                                <span class="link__text">
                                                    Ещё 13
                                                </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>
                                </div>
                                <!-- END subcats -->

                            </div>
                            <div class="cat cat_3">
                                <!-- link -->
                                <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                    Лампы
                                </a>
                                <!-- END link -->

                                <!-- img -->
                                <div class="cat__img">
                                    <img src="images/content/cats/cat4.png" alt="Светильники светодиодные"
                                         title="Светильники светодиодные" class="cat__pic">
                                </div>
                                <!-- END img -->

                                <div class="cat__bg"
                                     style="background-image: url(images/content/cats/catbg4.jpg);"></div>

                                <!-- subcats -->
                                <div class="subcats">
                                    <div class="subcats__blur"
                                         style="background-image: url(images/content/cats/catbg4.jpg);">
                                        <div class="cat__img">
                                            <img src="images/content/cats/cat4.png" alt="Светильники светодиодные"
                                                 title="Светильники светодиодные" class="cat__pic">
                                        </div>
                                    </div>
                                    <div class="subcats__content">
                                        <a role="link" href="category.php" title="Светильники светодиодные" class="subcats__title">
                                            Лампы
                                        </a>
                                        <div class="subcats__list">
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы"
                                               class="subcat subcat_link link">
                                                <span class="link__text">
                                                    Ещё 13
                                                </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <!-- END subcats -->

                            </div>
                            <div class="cat cat_3">
                                <!-- link -->
                                <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                    Лампы
                                </a>
                                <!-- END link -->

                                <!-- img -->
                                <div class="cat__img">
                                    <img src="images/content/cats/cat5.png" alt="Светильники светодиодные"
                                         title="Светильники светодиодные" class="cat__pic">
                                </div>
                                <!-- END img -->

                                <div class="cat__bg"
                                     style="background-image: url(images/content/cats/catbg5.jpg);"></div>

                                <!-- subcats -->
                                <div class="subcats">
                                    <div class="subcats__blur"
                                         style="background-image: url(images/content/cats/catbg5.jpg);">
                                        <div class="cat__img">
                                            <img src="images/content/cats/cat5.png" alt="Светильники светодиодные"
                                                 title="Светильники светодиодные" class="cat__pic">
                                        </div>
                                    </div>
                                    <div class="subcats__content">
                                        <a role="link" href="category.php" title="Светильники светодиодные" class="subcats__title">
                                            Лампы
                                        </a>
                                        <div class="subcats__list">
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы"
                                               class="subcat subcat_link link">
                                                <span class="link__text">
                                                    Ещё 13
                                                </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>
                                </div>
                                <!-- END subcats -->

                            </div>
                            <div class="cat lg-12">
                                <!-- link -->
                                <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                    Светильники светодиодные
                                </a>
                                <!-- END link -->

                                <div class="cat__bg"
                                     style="background-image: url(images/content/cats/catbg6.jpg);"></div>

                                <!-- subcats -->
                                <div class="subcats">
                                    <div class="subcats__blur"
                                         style="background-image: url(images/content/cats/catbg6.jpg);"></div>
                                    <div class="subcats__content">
                                        <a role="link" href="category.php" title="Светильники светодиодные" class="subcats__title">
                                            Светильники светодиодные
                                        </a>
                                        <div class="subcats__list">
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы"
                                               class="subcat subcat_link link">
                                                <span class="link__text">
                                                     Ещё 13
                                                </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>
                                </div>
                                <!-- END subcats -->

                            </div>
                            <div class="cat">
                                <!-- link -->
                                <a role="link" href="category.php" title="Светильники светодиодные" class="cat__link">
                                    Светильники светодиодные
                                </a>
                                <!-- END link -->


                                <div class="cat__bg"
                                     style="background-image: url(images/content/cats/catbg7.jpg);"></div>

                                <!-- subcats -->
                                <div class="subcats">
                                    <div class="subcats__blur"
                                         style="background-image: url(images/content/cats/catbg7.jpg);"></div>
                                    <div class="subcats__content">
                                        <a role="link" href="category.php" title="Светильники светодиодные" class="subcats__title">
                                            Светильники светодиодные
                                        </a>
                                        <div class="subcats__list">
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы" class="subcat">
                                                Энергосберигающие лампы
                                            </a>
                                            <a role="link" href="category.php" title="Энергосберигающие лампы"
                                               class="subcat subcat_link link">
                                                <span class="link__text">
                                                    Ещё 13
                                                </span>
                                                <svg class="link__svg">
                                                    <use xlink:href="images/icons/sprite.svg#right"></use>
                                                </svg>
                                            </a>

                                        </div>
                                    </div>
                                </div>
                                <!-- END subcats -->

                            </div>
                        </div>

                        <a role="link" href="catalog.php" class="cats__more link">
                            <span class="link__text">
                                Смотреть весь каталог
                            </span>
                            <svg class="link__svg">
                                <use xlink:href="images/icons/sprite.svg#right"></use>
                            </svg>
                        </a>
                    </div>
                </div>
                <div class="tab" id="tab2">
                    <div class="main-catalog">
                        <div class="folders">
                            <div class="row">
                                <div class="folder">
                                    <a role="link" href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                        <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                    </a>

                                    <div class="subfolders">
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder link" title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                            <svg class="link__svg">
                                                <use xlink:href="images/icons/sprite.svg#right"></use>
                                            </svg>
                                        </a>

                                    </div>
                                </div>
                                <div class="folder">
                                    <a role="link" href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                        <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                    </a>

                                    <div class="subfolders">
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder link" title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                            <svg class="link__svg">
                                                <use xlink:href="images/icons/sprite.svg#right"></use>
                                            </svg>
                                        </a>

                                    </div>
                                </div>
                                <div class="folder">
                                    <a role="link" href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                        <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                    </a>

                                    <div class="subfolders">
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder link" title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                            <svg class="link__svg">
                                                <use xlink:href="images/icons/sprite.svg#right"></use>
                                            </svg>
                                        </a>

                                    </div>
                                </div>
                                <div class="folder">
                                    <a role="link" href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                        <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                    </a>

                                    <div class="subfolders">
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder link" title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                            <svg class="link__svg">
                                                <use xlink:href="images/icons/sprite.svg#right"></use>
                                            </svg>
                                        </a>

                                    </div>
                                </div>
                                <div class="folder">
                                    <a role="link" href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                        <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                    </a>

                                    <div class="subfolders">
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder link" title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                            <svg class="link__svg">
                                                <use xlink:href="images/icons/sprite.svg#right"></use>
                                            </svg>
                                        </a>

                                    </div>
                                </div>
                                <div class="folder">
                                    <a role="link" href="category.php" class="folder__link" title="Светильники светодиодные">
                                        <span class="folder__img">
                                            <img src="/images/content/menu/menu1-active.png"
                                                 alt="Светильники светодиодные"
                                                 class="folder__pic">
                                        </span>
                                        <span class="folder__link-text">
                                          Светильники светодиодные
                                        </span>
                                    </a>

                                    <div class="subfolders">
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder" title="Светильники светодиодные">
                                            Энергосберигающие лампы
                                        </a>
                                        <a role="link" href="category.php" class="subfolder link" title="Светильники светодиодные">
                                        <span class="link__text">
                                            Ещё 13
                                        </span>
                                            <svg class="link__svg">
                                                <use xlink:href="images/icons/sprite.svg#right"></use>
                                            </svg>
                                        </a>

                                    </div>
                                </div>

                            </div>
                        </div>
                    </div>
                </div>
                <div class="tab" id="tab3">
                    <? include 'pages/common/actions.php' ?>
                </div>

            </div>
            <!-- END tabs -->
        </div>
    </section>
    <!-- END tabs -->

    <!-- brands -->
    <section class="main-brands">
        <div class="container">

            <h2 class="main-brands__title">
                Наши бренды
            </h2>

            <div class="row">

                <!-- main-brand__prev -->
                <div class="main-brands__prev">
                    <button role="button" class="arrow arrow_white main-brands__prev-arrow">
                       <svg class="arrow__svg">
                            <use xlink:href="images/icons/sprite.svg#left"></use>
                        </svg>
                    </button>
                </div>
                <!-- END main-brand__prev -->

                <!-- slider -->
                <div class="main-brands__slider">
                    <div class="swiper-container main-brands__container">
                        <div class="swiper-wrapper">
                            <? for ($i = 1; $i < 9; $i++): ?>
                                <a role="link" href="brand-select.php" title="Бренд<?= $i ?>" class="swiper-slide main-brand">
                                    <span class="main-brand__content">
                                        <img src="images/content/brands/brand<?= $i ?>.png" alt="Бренд<?= $i ?>"
                                             title="Бренд<?= $i ?>"
                                             class="main-brand__img">
                                    </span>
                                </a>
                            <? endfor; ?>
                        </div>
                    </div>
                </div>
                <!-- END slider -->

                <!-- main-brand__next -->
                <div class="main-brands__next">
                    <button role="button" class="arrow arrow_white main-brands__prev-arrow">
                       <svg class="arrow__svg">
                            <use xlink:href="images/icons/sprite.svg#right"></use>
                        </svg>
                    </button>
                </div>
                <!-- END main-brand__prev -->

            </div>

            <div class="main-brands__link">
                <a role="link" title="Смотреть все бренды" href="brands.php" class="link link_white">
                    <span class="link__text">
                        Смотреть все бренды
                    </span>
                    <svg class="link__svg">
                        <use xlink:href="images/icons/sprite.svg#right"></use>
                    </svg>
                </a>
            </div>

        </div>
    </section>
    <!-- END brands -->

    <!-- main-about -->
    <section class="main-about">
        <div class="container">

            <div class="main-about__top">
                <h2 class="main-about__title">
                    Что говорят о нас ?
                </h2>
                <div class="main-about__link">
                    <a role="link" href="reviews.php" class="link" title="Смотреть все отзывы">
                    <span class="link__text">
                        Смотреть все отзывы
                    </span>
                    <svg class="link__svg">
                        <use xlink:href="images/icons/sprite.svg#right"></use>
                    </svg>
                    </a>
                </div>
            </div>

            <div class="main-about__slider">

                <!-- wrapper -->
                <div class="main-about__slider-wrap">

                    <!-- swiper -->
                    <div class="swiper-container main-about__container">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide rev">
                                <div class="rev__img">
                                    <img src="images/content/about/about1.jpg" alt="Отзыв1" title="Отзыв1"
                                         class="rev__pic">
                                </div>
                                <div class="rev__content">
                                    <div class="rev__title">
                                        Петрова Ирина, г. Москва
                                    </div>
                                    <div class="rev__desc">
                                        Повседневная практика показывает, что рамки
                                        и место обучения кадров обеспечивает широкому кругу (специалистов) участие в
                                        формировании существенных финансовых и административных условий. С другой
                                        стороны
                                        дальнейшее развитие различных форм деятельности играет важную роль
                                        в формировании системы обучения кадров, соответствует насущным потребностям.
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide rev">
                                <div class="rev__img">
                                    <img src="images/content/about/about2.jpg" alt="Отзыв2" title="Отзыв2"
                                         class="rev__pic">
                                </div>
                                <div class="rev__content">
                                    <div class="rev__title">
                                        Тихонов Александр, г. Москва
                                    </div>
                                    <div class="rev__desc">
                                        Повседневная практика показывает, что рамки
                                        и место обучения кадров обеспечивает широкому кругу (специалистов) участие в
                                        формировании существенных финансовых и административных условий. С другой
                                        стороны дальнейшее развитие различных форм деятельности играет важную роль
                                        в формировании системы обучения кадров, соответствует насущным потребностям.
                                    </div>
                                </div>
                            </div>
                            <div class="swiper-slide rev">
                                <div class="rev__img">
                                    <img src="images/content/about/about1.jpg" alt="Отзыв1" title="Отзыв1"
                                         class="rev__pic">
                                </div>
                                <div class="rev__content">
                                    <div class="rev__title">
                                        Петрова Ирина, г. Москва
                                    </div>
                                    <div class="rev__desc">
                                        Повседневная практика показывает, что рамки
                                        и место обучения кадров обеспечивает широкому кругу (специалистов) участие в
                                        формировании существенных финансовых и административных условий. С другой
                                        стороны
                                        дальнейшее развитие различных форм деятельности играет важную роль
                                        в формировании системы обучения кадров, соответствует насущным потребностям.
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- END swiper -->

                    <!-- nav -->

                    <div class="main-about__prev">
                        <div class="main-about__prev-wrap">
                            <div class="main-about__arrow">
                            </div>
                        </div>
                        <button role="button" class="arrow arrow_about">
                            <svg class="arrow__svg">
                                <use xlink:href="images/icons/sprite.svg#left"></use>
                            </svg>
                        </button>
                    </div>
                    <div class="main-about__next">
                        <div class="main-about__next-wrap">
                            <div class="main-about__arrow">
                            </div>
                        </div>
                        <button role="button" class="arrow arrow_about">
                            <svg class="arrow__svg">
                                <use xlink:href="images/icons/sprite.svg#right"></use>
                            </svg>
                        </button>
                    </div>
                    <!-- END nav -->

                </div>
                <!-- END wrapper -->

                <!-- pagination -->
                <div class="swiper-pagination main-about__pagination"></div>
                <!-- END pagination -->

            </div>

            <div class="main-about__bottom">
                <div class="main-about__link">
                    <a role="link" href="reviews.php" class="link" title="Смотреть все отзывы">
                    <span class="link__text">
                        Смотреть все отзывы
                    </span>
                    <svg class="link__svg">
                        <use xlink:href="images/icons/sprite.svg#right"></use>
                    </svg>
                    </a>
                </div>
            </div>

            <!-- news -->
            <div class="main-news">
                <!-- top -->
                <div class="main-news__top">
                    <h2 class="main-news__title">
                        Новости
                    </h2>
                    <div class="main-news__link">
                        <a role="link" href="events.php" class="link" title="Смотреть все новости">
                            <span class="link__text">
                                Смотреть все новости
                            </span>
                            <svg class="link__svg">
                                <use xlink:href="images/icons/sprite.svg#right"></use>
                            </svg>
                        </a>
                    </div>
                </div>
                <!-- END top -->

                <!-- list -->
                <div class="main-news__list">
                    <div class="row">
                        <? for ($i = 1; $i < 5; $i++): ?>
                            <a role="link" href="event.php" title="Новость1" class="main-news__item">
                            <span class="main-news__pic">
                                <img src="images/content/news/news<?= $i ?>.jpg" title="Новость<?= $i ?>"
                                     alt="Новость<?= $i ?>" class="main-news__img">
                            </span>

                                <span class="main-news__date">
                                13.05.2018
                            </span>
                                <span class="main-news__title">
                                Новый ассортимент светодиодных ламп
                            </span>
                            </a>
                        <? endfor; ?>
                    </div>
                </div>
                <!-- END list -->

                <!-- bottom -->
                <div class="main-news__bottom">
                    <div class="main-news__link">
                        <a role="link" href="events.php" class="link" title="Смотреть все новости">
                            <span class="link__text">
                                Смотреть все новости
                            </span>
                            <svg class="link__svg">
                                <use xlink:href="images/icons/sprite.svg#right"></use>
                            </svg>
                        </a>
                    </div>
                </div>
                <!-- END bottom -->
            </div>
            <!-- END news -->

        </div>
    </section>
    <!-- END main-about -->

</main>